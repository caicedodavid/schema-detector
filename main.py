...
from libs.transform_functions import parse_element, generate_schema, convert_schema_to_table_schema
...
def parse_arguments() -> Tuple[Namespace, List[str]]:
    ...
    parser.add_argument('--outputDataset', required=True, help='Dataset name to write records')
    parser.add_argument('--outputTable', required=True, help='Table name to write records')
    parser.add_argument('--tempGcsLocation', required=True, help='GCS bucket location to write temporary results')
...
def run_pipeline(opts:Namespace, pipeline_opts: List[str]) -> None:
    ...
    with beam.Pipeline() as pipeline:
        ...
        (
            schema 
            | "Update table Schema" >> beam.ParDo(UpdateSchema(opts.project, opts.outputDataset, opts.outputTable))
        )
        (
            parsed_records
